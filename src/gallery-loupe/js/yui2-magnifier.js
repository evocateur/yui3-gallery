...
YAHOO.extension.Magnifier = function (anchor, magnifierCfg, lensCfg, cursorCfg) {
    ...
    if (typeof magnifierCfg.image === 'undefined') {
        magnifierCfg.image = anchor.getElementsByTagName("img")[0];
    }
    ...
    // set up lens
    if (typeof lensCfg === 'undefined') {
        lensCfg = {};
    }
    if (typeof lensCfg.showDelay === 'undefined') {
        lensCfg.showDelay = this.cfg.getProperty("showDelay");
    }
    if (typeof lensCfg.imageUrl === 'undefined') {
        lensCfg.imageUrl = anchor.href;
    }
    if (typeof lensCfg.context === 'undefined') {
        lensCfg.context = [anchor, 'tl', 'tr'];
    }
    ...
    // set up cursor panel
    if (typeof cursorCfg === 'undefined') {
        cursorCfg = {};
    }
    if (typeof cursorCfg.showDelay === 'undefined') {
        cursorCfg.showDelay = this.cfg.getProperty("showDelay");
    }

    var cursorBoxToLensRatio = 1 / this.getMagnificationRatio();

    cursorCfg.width  = (parseInt(this.lens.cfg.getProperty("width"),  10) * cursorBoxToLensRatio) + 'px';
    cursorCfg.height = (parseInt(this.lens.cfg.getProperty("height"), 10) * cursorBoxToLensRatio) + 'px';
...
YAHOO.extension.CursorPanel = function (el, cpCfg) {
    ...
    //override defaults
    if (typeof cpCfg.visible === 'undefined') {
        cpCfg.visible = false;
    }
    if (typeof cpCfg.draggable === 'undefined') {
        cpCfg.draggable = false;
    }
    if (typeof cpCfg.close === 'undefined') {
        cpCfg.close = false;
    }
    if (typeof cpCfg.underlay === 'undefined') {
        cpCfg.underlay = 'none';
    }
...
YAHOO.lang.extend(YAHOO.extension.CursorPanel, YAHOO.widget.Panel, {
    ...
    getBoundary: function () {
        return this.cfg.getProperty('extendedBoundary') || this.cfg.getProperty('parentElement');
    },
    ...
    _setBoundaryVars: function () {
        this.boundary = this.cfg.getProperty('extendedBoundary') || this.cfg.getProperty('parentElement');
    ...
    setPosition: function (coords) {
        var x = coords[0] - (this.realWidth  / 2),
    ...
    handleMouseOut: function (evt) {
        ...
        var ignoreNodes = [this.getBoundary()]
            .concat(YAHOO.util.Dom.getElementsBy(function () { return true; }, null, this.getBoundary()))
            .concat(this.element)
            .concat(YAHOO.util.Dom.getElementsBy(function () { return true; }, null, this.element));
        var relatedTarget = YAHOO.util.Event.getRelatedTarget(evt),
            i = 0,
            iLen = ignoreNodes.length;
        for (; i < iLen; i++) {
